...
void* Heap_allocate_OVERRIDE(Heap* self, u32 size, int align)
{
	...
	if (self == Memory::currentHeapPtr)
		Log() << "ALLOC RAM: " << self->vMemoryLeft() << " bytes free. " << self->vMaxAllocatableSize(align) << " bytes allocateable.\n";
...
void Heap_deallocate_OVERRIDE(Heap* self, void* ptf)
{
	...
	if (self == Memory::currentHeapPtr)
		Log() << "FREE  RAM: " << self->vMemoryLeft() << " bytes free. " << self->vMaxAllocatableSize(4) << " bytes allocateable.\n";
}
