...
namespace CollisionViewer {
	...
	void renderActiveCollider(const ActiveCollider& u, Flags flags) {
		...
		case ActiveCollider::Shape::Rectangle:
		...
		case ActiveCollider::Shape::Round:
		...
		case ActiveCollider::Shape::TrapezoidH:
		case ActiveCollider::Shape::TrapezoidV:
		{
			Vec2 pos(box.x, box.y);
			fx32 points[4];

			points[0] = u.trapH.topLeft;
			points[1] = u.trapH.topRight;
			points[2] = u.trapH.botLeft;
			points[3] = u.trapH.botRight;

			if (u.shape == ActiveCollider::Shape::TrapezoidH) {
